...
describe('useLayer', () => {
  ...
  it('레이어 추가/삭제', () => {
    const { unmount } = renderHook(() => useOlLayer(layer, options));
  ...
  it('레이어 변경', () => {
    ...
    const { rerender } = renderHook(() => useOlLayer(prevLayer, options));
    renderHook(() => useOlLayer(layer, options));
    renderHook(() => useOlLayer(layer, options));
    renderHook(() => useOlLayer(layer, options));
  ...
  it('레이어의 map 속성에 값이 있을 경우, 레이어 목록에 추가하지 않는다', () => {
    const { unmount } = renderHook(() => useOlLayer(layer, { ...options, map }));
