...
import { useOlObservable } from './useOlObservable';
...
describe('useObservable', () => {
  ...
  it('이벤트 함수를 등록하고, 언마운트시 해제', () => {
    ...
    const { unmount } = renderHook(({ target, props }) => useOlObservable(target, props), {
      initialProps,
    });
  ...
  it('이벤트 함수가 동일하면 새로 등록하지 않는다', () => {
    ...
    const { rerender } = renderHook(({ target, props }) => useOlObservable(target, props), {
      initialProps,
    });
